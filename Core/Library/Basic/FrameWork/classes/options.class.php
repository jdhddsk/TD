    ...
    public function set_options() {
      ...
      if( wp_verify_nonce( csf_get_var( 'csf_options_nonce'. $this->unique ), 'csf_options_nonce' ) ) {
        ...
        if( ! empty( $transient['csf_import_data'] ) ) {
          ...
          $this->notice = esc_html__( '已经重置此选项卡的所有选项为默认值。', 'csf' );
      ...
      echo '<div class="csf csf-options'. $theme . $class . $wrapper_class .'" data-slug="'. $this->args['menu_slug'] .'" data-unique="'. $this->unique .'">';
        ...
        echo '<div class="csf-header-inner">';
          ...
          echo '<div class="csf-header-right">';
            ...
            echo ( $has_nav && $this->args['show_all_options'] ) ? '<div class="csf-expand-all" title="'. esc_html__( 'show all options', 'csf' ) .'"><i class="iconoir-menu" style="height: 30px;display: flex;align-items: center;"></i></div>' : '';

            echo ( $this->args['show_search'] ) ? '<div class="csf-search"><input type="text" name="csf-search" placeholder="'. esc_html__( '搜索设置项...', 'csf' ) .'" autocomplete="off" /></div>' : '';

            echo '<div class="csf-buttons">';
            echo '<input type="submit" name="'. $this->unique .'[_nonce][save]" class="button button-primary csf-save'. $ajax_class .'" value="'. esc_html__( '保存', 'csf' ) .'" data-save="'. esc_html__( '保存中...', 'csf' ) .'">';
            echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( '重置此选项卡', 'csf' ) .'" data-confirm="'. esc_html__( '确定要重置此选项卡内所有内容？请三思！', 'csf' ) .'">' : '';
            echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button button-secondary csf-warning-primary csf-reset-all csf-confirm" value="'. esc_html__( '重置', 'csf' ) .'" data-confirm="'. esc_html__( '确定重置？请三思！', 'csf' ) .'">' : '';
            echo '</div>';
        ...
        if( ! empty( $this->args['show_footer'] ) ) {
          ...
          echo '<div class="csf-buttons">';
          echo '<input type="submit" name="csf_transient[save]" class="button button-primary csf-save'. $ajax_class .'" value="'. esc_html__( '保存', 'csf' ) .'" data-save="'. esc_html__( '保存中...', 'csf' ) .'">';
          echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( '重置此选项卡', 'csf' ) .'" data-confirm="'. esc_html__( '确定要重置此选项卡内所有内容？请三思！', 'csf' ) .'">' : '';
          echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button button-secondary csf-warning-primary csf-reset-all csf-confirm" value="'. esc_html__( '重置', 'csf' ) .'" data-confirm="'. esc_html__( '确认重置？请三思！', 'csf' ) .'">' : '';
          echo '</div>';

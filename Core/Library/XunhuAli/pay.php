...
$money = $_GET['money'];
$name = $_GET['name'];
$gcz_credit = $_GET['gcz_credit'];
require_once 'api.php';
$trade_order_id = time();//商户网站内部ID，此处time()是演示数据
$appid          = get_option('gcz_options')['gcz-ali-pay-xunhupay']['gcz-ali-pay-xunhupay-appid'];
$appsecret      = get_option('gcz_options')['gcz-ali-pay-xunhupay']['gcz-ali-pay-xunhupay-appsecret'];
...
$data = array(
    ...
    'wap_url'       => home_url(),      //网站域名，H5支付必填
    'wap_name'      => home_url(),      //网站域名，或者名字，必填，长度32或以内 H5支付必填
    'total_fee'     => $money,          //人民币，单位精确到分(测试账户只支持0.1元内付款)
    'title'         => $name,           //必须的，订单标题，长度32或以内
    'time'          => time(),          //必须的，当前时间戳，根据此字段判断订单请求是否已超时，防止第三方攻击服务器
    'notify_url'    => GCZ_URI.'/Core/Library/XunhuAli/notify.php', //必须的，支付成功异步回调接口
    'return_url'    => GCZ_URI.'/Core/Classes/PaySuccess.php?user='.get_current_user_id().'&order='.$trade_order_id,//必须的，支付成功后的跳转地址

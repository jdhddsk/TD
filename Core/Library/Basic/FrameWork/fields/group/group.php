  ...
  class CSF_Field_group extends CSF_Fields {
    ...
    public function render() {
      ...
      $args = wp_parse_args( $this->field, array(
        ...
        'min'                    => 0,
        'fields'                 => array(),
        'button_title'           => esc_html__( '新建', 'csf' ),
      ...
      if( ! empty( $this->parent ) && preg_match( '/'. preg_quote( '['. $this->field['id'] .']' ) .'/', $this->parent ) ) {
        ...
        echo '<div class="csf-cloneable-item csf-cloneable-hidden">';
          ...
          echo '<i class="csf-cloneable-sort iconoir-arrow-separate-vertical"></i>';
          echo '<i class="csf-cloneable-clone iconoir-copy"></i>';
          echo '<i class="csf-cloneable-remove csf-confirm iconoir-cancel" data-confirm="'. esc_html__( '确定要删除该项目吗？删除后无法找回，请三思', 'csf' ) .'"></i>';
          echo '</div>';
        ...
        echo '<div class="csf-cloneable-alert csf-cloneable-max">'. esc_html__( '你不能添加超过', 'csf' ) .' '. $args['max'] .'</div>';
        echo '<div class="csf-cloneable-alert csf-cloneable-min">'. esc_html__( '你不能减少超过', 'csf' ) .' '. $args['min'] .'</div>';
